...
from measurement import init_drone, state_meas, att_quat#, pwm_req, pwm_set, set_rpyt, req_rpyt
...
def simulation():
    ...
    t0 = 0
    state_init = np.copy(init_st)                                # initial state
    state_target = np.copy(targ_st)                              # target state
    ...
    state_error = get_error2(state_init[0:3] - state_target[0:3])
    ...
    while (state_error > 1e-1) and (mpc_iter < sim_Smax):
        ...
        mpc_iter = mpc_iter + 1
        state_error = get_error2(np.subtract(state_init[0:3], state_target[0:3]))
...
def onboard(scf):
    ...
    while (state_error > 1e-1) and (mpc_iter  < sim_Smax):
        ...
        # Generate API setpoint
        roll, pitch, yawRate, thrust_norm = calc_thrust_setpoint(X0[:, 1], u[:, 1])
        print(f'Soln setpoints {mpc_iter}: at {round(t0,3)} s\r') #{roll}, {pitch}, {yawRate}, {thrust_norm}  
        setpoints = np.vstack( (setpoints, np.array([roll, pitch, yawRate, thrust_norm], dtype="object")))
